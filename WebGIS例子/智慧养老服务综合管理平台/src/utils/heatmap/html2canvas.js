    ...
    var prefixLinearGradient = function(tokens) {
        ...
        parseFunctionArgs(tokens).forEach(function(arg, i) {
            if (i === 0) {
                var firstToken = arg[0]
                if (
                    firstToken.type === TokenType.IDENT_TOKEN && ['top', 'left', 'right', 'bottom'].indexOf(firstToken.value) !== -1
                ) {
                    angle$1 = parseNamedSide(arg)
                    return
                } else if (isAngle(firstToken)) {
        ...
        Logger.prototype.debug = function() {
            ...
            if (this.enabled) {
                ...
                if (
                    ...
                    // eslint-disable-next-line no-console
                    console.debug.apply(
                        console, [this.id, this.getTime() + 'ms'].concat(args)
                    )
                } else {
        ...
        if (
            ...
            if (hasIntrinsicValue(intrinsicProportion)) {
                ...
                return targetRatio < intrinsicProportion !==
                    (first.value === BACKGROUND_SIZE.COVER) ? [bounds.width, bounds.width / intrinsicProportion] : [bounds.height * intrinsicProportion, bounds.height]
            }
                                    ...
                                    switch (_f.label) {
                                        ...
                                        case 5:
                                            if (isLinearGradient(backgroundImage)) {;
                                                ...
                                                (height = _d[4])
                                                position =
                                                    backgroundImage.position.length === 0 ? [FIFTY_PERCENT] :
                ...
                switch (_b.label) {
                    case 0:
                        ...
                        defaultOptions = {
                            backgroundColor: '#ffffff',
                            cache: opts.cache ?
                                opts.cache : CacheStorage.create(instanceName, resourceOptions),
                            logging: true,
